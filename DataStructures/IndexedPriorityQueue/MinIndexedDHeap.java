...
public class MinIndexedDHeap <T extends Comparable<T>> {
    ...
    //The position map that maps key indices to where the position of that 
    //key is represented in the priority queue in the domain [0, sz)
    public final int[] pm;

    //The inverse map that stores the indices of the keys in the range
    //[o, sz) which make up the priority queue. It should be noted that im and
    //pm are inverses of each other
    public final int[] im;

    //The values associated with the keys. It is very important to note that this
    //array is indexed by the key indices (ki)
